...
// O PROMPT INTEGRAL (Mantenha exatamente como você escreveu)
...
app.get("/", (req, res) => {
  res.json({ message: "Rav Ohr Online" });
});
...
app.post("/chat", async (req, res) => {
  ...
  try {
    // Usando o modelo 3.1 Flash Lite que vimos no seu print
    const model = genAI.getGenerativeModel({ 
      model: "gemini-1.5-flash", // No SDK de Node, o 3.1 costuma responder por este ID ou 'gemini-1.5-flash-latest'
    ...
    // IMPORTANTE: Retornar o campo 'reply' para o seu App/Worker ler
    res.json({ reply: response.text() });

  } catch (error) {
    console.error("Erro no Railway:", error);
    res.status(500).json({ error: "Erro na conexão com o Rav." });
  }
});
