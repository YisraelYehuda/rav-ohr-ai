...
app.get("/", (req, res) => {
  res.json({ status: "Rav Ohr Online", connection: "Baruch HaShem" });
});
...
app.post("/chat", async (req, res) => {
  ...
  try {
    // Definindo o modelo exato do Studio
    const model = genAI.getGenerativeModel({ 
      model: "gemini-3.1-flash-lite",
      systemInstruction: SYSTEM_PROMPT 
    });

    // Chamada configurada para emular o Studio perfeitamente
    ...
    res.status(500).json({ error: "Ocorreu um erro no servidor do Rav." });
  }
});

app.listen(PORT, '0.0.0.0', () => {
  console.log(`Rav Ohr operando na porta ${PORT}`);
});
